...
`operational.cadence` in the artifact at pin `v20260922-ff164d5`:
...
> ⚠️ **This list mirrors `operational.first_data` in the artifact at pin `v20260922-ff164d5`.** It is
...
```
install v20260922-ff164d5  →  uis dagster run klass_refresh  →  uis dagster run transform_and_publish
```
...
🟠 **The sequence here is HAND-CARRIED, but not for the reason this page gave.**

🔴 **CORRECTION.** This block previously said *"the artifact does not contain it … nothing in
the install definition mentions `klass_refresh`."* **That was wrong.** The pinned artifact names
`klass_refresh` **twice** — `first_data.jobs` lists it **second, before `transform_and_publish`**,
and the monthly cadence row names it. **A first install following `first_data.jobs` already gets the
correct order.** Worse, this repository's own verifier prints `first_data.jobs (6, order matters):
annual_sources_refresh, klass_refresh, …` on **every** pin run, so the refutation was on screen each
time the claim was repeated. Corrected on urb-agents#1373.

✅ **The real gap was narrower and is now closed upstream: the artifact had no concept of an
UPGRADE at all.** `first_data` describes a *first* install; nothing addressed a host that already has
data. `operational.upgrade` was merged at `6cb28f4` and states the **rule** rather than another job
list — the default for an upgrade is the transform alone, and the exception is a release that changes
**what an ingest fetches** (its window, its filters, its dimensions), because *"the transform cannot
recover rows that were never requested."* 🔵 **A rule with a stated trigger survives a release;
a per-release job list has to be maintained by the person most likely to have just forgotten the
step.**

⚠️ **So this sequence stays hand-carried until a build carrying `operational.upgrade` is
PINNED — and that has not happened yet.** The field was merged at `6cb28f4`, which **has been built
and not deployed**; the pinned build is two commits behind it and does not carry the field
(`operational.upgrade` and `operational.crons` both absent, verified by parsing the pinned artifact
rather than grepping it). 🔵 **"The field exists in the artifact" and "the pinned artifact has
the field" are different claims**, and this page was given the first one (urb-agents#1373) and told
the difference explicitly a release later (#1374). **Treat this block as needing re-checking rather
than trusted until the condition is actually met.**
