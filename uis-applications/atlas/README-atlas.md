...
are a latency warning that had existed only in bus messages and a website page:

| | before | after |
|---|---|---|
| `definitions.brreg_enhet.description` | 958 chars | **1,730** |
| `count=exact`, `Prefer`, `count=planned`, CORS | all **absent** | all **present** |

- **A filtered `count=exact` takes 12–33 s**, against sub-second without the header.
- ⚠️ **Inside a browser `fetch` that is a timeout, not a slow answer** — in the consumer's own
  words, *a request that never completes is indistinguishable from a blocked one*, so this reads as
  CORS or a dead endpoint and gets debugged as the wrong problem.
...
**`min_value` and `max_value` were computed over every kommune the fact table holds, while the counts
printed next to them were scoped to active non-sentinel kommuner.** So **33 of 222 series** published
a range over kommuner the relation does not serve — a number that looked like data and described a
different population from the one it was labelled with.

| | before 00:43Z | after 00:57Z |
|---|---|---|
| series where published min/max disagreed with the rows | **33** | **0** |

| series | before | after |
|---|---|---|
| `ssb-06913` `Folkemengde` | 0 .. 728714 | **219** .. 728714 |
| `ssb-06913` `Dode` | 0 .. 4033 | **2** .. 4033 |
| `ssb-09429` `PersonerProsent_01` | 0 .. 49.1 | **15.1** .. 49.1 |
| `ssb-crime-tables` `0__AnmLovbrPer1000` | 3.9 .. 236 | **5.4 .. 57.3** |

Measured from outside by paging **all 79,254 fact rows** with the count asserted, folding per series
and comparing against all 222 — and independently on the host by the operator.

🔵 **The two things that could have gone wrong and did not, which are the interesting part:**

- **`ssb-06913` `Levende` still has `min = 0`**, max 9287, 357 kommuner. A fix that excluded zeros
  along with sentinels would have produced a **tidier** table, passed a casual read, and **silently
  deleted the fact that a kommune had no live births.** The consumer checked `Levende` *before*
  flagging every zero, and that restraint became the acceptance test.
- **`ssb-12063` `KOSfritidredleie0000` keeps its row**, with `min` NULL, `max` NULL and count 0 —
  **present and empty, not dropped.** An absent row and an empty row answer different questions.

⚠️ **And *"the other 189 series are unchanged"* here is a COMPARISON, not an absence of
noticing.** All 222 were snapshotted before the deploy and diffed numerically after, so `13` and
`13.0` could not read as a change. That distinction is why the claim is repeated on this page at all.

✅ **`v20260922-6cb28f4` was measured the same way, and more strictly:** the 222-series snapshot
taken *before* the job was diffed **byte for byte** against the one after — all 222 rows identical,
*"not a value, not a count, not a year."* 🔵 **A snapshot taken before is a different class of
evidence from a spot-check taken after**, and it is the difference between "nothing moved" and "I
did not see anything move".
...
🔴 **LEFT UNRUN, THE MIDDLE STEP DOES NOT STAY MISSING — IT SELF-HEALS, WHICH IS WORSE.**
`klass_refresh` has a cron of `0 1 1 * *`: **01:00 on the 1st of each month.** So `3e72760` was not
inert forever. It was inert for **nine days**, and on **1 October** the 7,296-row FK failure would
have cleared **on its own**, with nobody having changed anything and no record of why. ⚠️ **A
defect that repairs itself on a schedule is not a mild defect** — it is one that cannot be found by
noticing it stopped happening. atlas had been treating the missed re-ingest as a one-off ordering
mistake it kept repeating; the class self-heals, so it only became visible because four parties
looked at the same source on the same night. **That is not a repeatable detection method**, which is
why the trigger now lives in the artifact rather than only the remedy (`self_healing_warning`, urb-agents#1373).

🔵 **Both halves of that were already on this page, 360 lines apart.** The cadence table above
says `klass_refresh` runs on the 1st at 01:00; this section says the fix needs `klass_refresh`. The
conclusion — *"then an operator who skips it gets a silent repair up to a month later"* — needed no
new information and was not drawn. **Same shape as the check-note contradiction three pins ago: two
sections that together state something neither states alone.**
...
| | the artifact's rule |
|---|---|
| **default for an upgrade** | `transform_and_publish` **alone** — *"an upgrade already has data"* |
| **the exception** | a release that changes **what an ingest fetches** — its date window, its filters, its dimensions |
| **why** | *"the transform cannot recover rows that were never requested, so that source's ingest job must run FIRST"* |
| **then** | the owning ingest job → `transform_and_publish` → `transform_checks` |
| **if you skip it** | *"an ingest-window change left unrun is repaired by that source's next scheduled cron — up to a month for `klass_refresh`. The symptom disappears on its own and the cause is never recorded. Re-run it deliberately."* |

🔵 **That is a RULE with a stated trigger, not another job list — which is why it will
outlive this pin.** A per-release list of jobs has to be maintained by the person most likely to have
just forgotten the step; a rule asking *"did this release change what an ingest fetches?"* keeps
working on releases nobody has written yet. **The Klass sequence above is now an instance of this
rule rather than a special case**, which is the whole reason it was worth waiting for.

⚠️ **Two pointers in that field do not resolve, and this page substitutes its own.** The field
says *"`operational.crons` above maps every job to what it covers"* — **there is no
`operational.crons` key**; the map is `operational.cadence`, which is the cadence table further up
this page. And it ends the sequence with **`transform_checks`, which is not in that map at all** (nor
in `manual_only` or `unscheduled`). Both verified by walking the parsed artifact. Reported on
urb-agents#1377 — neither changes what an operator should do, and both would send a reader looking
for something that is not there.

🔴 **Kept as the record: this page was wrong about the artifact for three pins.** It said
*"the artifact does not contain it … nothing in the install definition mentions `klass_refresh`."*
The artifact named it **twice** all along — `first_data.jobs` lists it **second, before
`transform_and_publish`**, and the monthly cadence row names it, so **a first install following
`first_data.jobs` already got the right order.** This repository's own verifier prints that job list
on **every** pin run, so the refutation was on screen each time the claim was repeated. What was
genuinely missing was narrower: **no concept of an UPGRADE at all** — `first_data` describes a
*first* install and nothing addressed a host that already has data. Corrected on urb-agents#1373.
...
🔴 **And what the check asks is not everything worth asking.** Three scope defects were found
on 2026-09-21–22 — counts scoped differently from the range beside them, a null filter scoped
differently from its subject, and `latest_year` scoped differently from the counts. **All three were
found from outside, by consumers. None was caught by a gate**, and the suite reported 990 pass / 18
fail before and after each one. ⚠️ **A green check means the questions it asks were answered
well**, which is a narrower statement than "the data is right". A gate for the class is being written
upstream; until it exists, treat a consumer noticing something as the detection mechanism it has
actually been.
