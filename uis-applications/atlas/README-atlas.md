...
⚠️ **This note previously said it would retire "with the next pin". That pin has now happened and
the note is still here.** The promised fix separates the two cases — **WARN** for marts built before
but the view missing (an upgrade waiting on a transform, which self-heals) and **CANNOT** for marts
never built (nothing has run here). At `v20260921-fcf78e6` the artifact's own operator text is
**byte-identical to the previous pin** and still documents only `CANNOT (exit 2)`, with no WARN case.
That is not proof the fix is absent — the check's code lives in the image, which this repository
cannot inspect — but it is no evidence that it landed, and **a retirement condition shaped like a
date retires on the date whether or not the thing was fixed.** The condition is therefore restated
as a measurement: **this note retires when an operator reports `uis template check atlas` returning
WARN, or exit 0, on an install whose transform has not yet run.** Until then, treat the item as true
of the pinned build. The old behaviour rested on an assertion that a missing view *"means no
transform has ever run here"*, which is false on every upgrade.
