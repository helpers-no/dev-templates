...
⚠️ **So this sequence stays hand-carried only until a build carrying `operational.upgrade` is
pinned** — `v20260921-a657f4f` does not have it (`operational.upgrade` and `operational.crons` are
both absent; verified by parsing the pinned artifact, not by grepping it). **Treat it as needing
re-checking rather than trusted until then.**
