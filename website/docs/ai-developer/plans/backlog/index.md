...
|----------|------|---------|
| [Investigate: CI + Deployment Targets as First-Class Template Metadata](INVESTIGATE-ci-and-deployment-targets.md) | Make CI workflow + deployment target first-class fields in `template-info.yaml`, consumed by both documentation generators and actual deployment tooling, so that (a) the implicit `.github/workflows/` convention becomes explicit, (b) Azure and future deployment targets slot in without a schema rewrite, and (c) the template page renders a concrete "builds via X, deploys to Y" line instead of leaving the reader to guess. | 2026-04-17 |
