...
description: "The Norwegian NGO sector in one place, served as a REST API with its Dagster pipelines"
---
...
<TemplateHeader
  ...
  description="The Norwegian NGO sector in one place, served as a REST API with its Dagster pipelines"
  abstract={"Atlas brings the Norwegian NGO sector together in one place. Installing it with UIS brings up the whole application under one name: a PostgreSQL database with its migrations, a Dagster code location for the ingestion pipelines, and a PostgREST API over the resulting api_v1 views. Exports the API url for other applications to consume."}
