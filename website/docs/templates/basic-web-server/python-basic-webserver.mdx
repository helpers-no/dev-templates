...
<TemplateEnvironment
  requires={null}
  params={null}
  quickstart={{"title":"Run the Flask app","commands":["uv venv","uv pip install -r requirements.txt","uv run python app/app.py"],"note":"Flask runs on port 3000. VS Code auto-forwards the port — click the globe icon in the Ports tab.\n"}}
