...
<TemplateEnvironment
  requires={null}
  params={null}
  quickstart={{"title":"Run the Go server","commands":["go run main.go"],"run":"go run main.go","note":"Server runs on port 3000. VS Code auto-forwards the port.\n"}}
...
flowchart LR
    subgraph dct["DCT devcontainer"]
        dev(["Developer"])
        dev_golang["dev-golang"]
        app["Go Basic Webserver"]
    end
    subgraph k8s["Local Kubernetes Cluster (Test environment)"]
        argo["ArgoCD"]
        pod["Go Basic Webserver pod"]
    end
    subgraph gh["GitHub"]
        repo["repo"]
