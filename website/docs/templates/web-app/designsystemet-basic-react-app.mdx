...
<TemplateEnvironment
  requires={null}
  params={null}
  quickstart={{"title":"Run the Vite dev server","commands":["npm install","npm run dev"],"run":"npm run dev","note":"Vite runs on port 5173. VS Code auto-forwards the port — click the globe icon in the Ports tab.\n"}}
...
flowchart LR
    subgraph dct["DCT devcontainer"]
        ...
        app["Designsystemet Basic React App"]
    end
    subgraph k8s["Local Kubernetes Cluster (Test environment)"]
        argo["ArgoCD"]
        pod["Designsystemet Basic React App pod"]
    end
    subgraph gh["GitHub"]
        repo["repo"]
