...
# Background color — auto-detected from image if not specified
BG_COLOR=""
...
if [[ -z "$BG_COLOR" ]]; then
    # Sample a pixel near the bottom-right corner to get the actual background color
    SAMPLE_X=$((WIDTH - 2))
    SAMPLE_Y=$((HEIGHT - 2))
    BG_COLOR=$(convert "$INPUT_FILE" -crop "1x1+${SAMPLE_X}+${SAMPLE_Y}" -format "%[pixel:p{0,0}]" info: | sed 's/srgba/rgba/' | sed 's/srgb/rgb/')
    echo "  Auto-detected background color: $BG_COLOR"
fi
