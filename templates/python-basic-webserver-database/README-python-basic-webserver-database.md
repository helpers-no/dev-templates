...
The shipped `.vscode/settings.json` enables `python-envs.alwaysUseUv` so VS Code's Python extension uses `uv` for package operations and works seamlessly with the venv.
...
├── .vscode/
│   └── settings.json                               # python-envs.alwaysUseUv: true
